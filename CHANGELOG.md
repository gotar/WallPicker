...
### Changed
- Wallhaven 2nd tab is now more diverse: default purity `SFW -> SFW+Sketchy (110)`, empty `date_added` searches are auto-diversified (60% random tag from 15-tag pool, 30% random/favorites/views/toplist with seed) so results are not monothematic; explicit queries/filters unchanged
...
### Changed
- Single-binary interface: `wallpicker set <path>` / `wallpicker current` /
  `-v` / `-h` run headless from the same binary; bare `wallpicker` launches
  the GUI. `wallpicker-cli` remains as an alias.
...
### Added
- `-v/--version` and `-h/--help` on both `wallpicker` (GUI) and
  `wallpicker-cli`; help documents all commands and options

## [2.5.9] - 2026-08-24

### Added
- Headless CLI sharing the GUI wallpaper pipeline:
  `wallpicker-cli set <path>` / `wallpicker-cli current`

### Fixed
- Logging is now configured from the GUI entry point as well: the packaged
  `/usr/bin/wallpicker` bypasses launcher.py, so installed builds produced
  no logs

## [2.5.8] - 2026-08-24

### Added
- Rotating log file at `~/.cache/wallpicker/logs/wallpicker.log` (INFO+,
  WARNING+ also on stderr) so issues can be diagnosed after the fact
